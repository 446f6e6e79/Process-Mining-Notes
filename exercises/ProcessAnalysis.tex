...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/exercises/proc_analysis/q_a-1.png}
...
\begin{table}[H]
    ...
    \resizebox{0.8\textwidth}{!}{%
        \begin{tabular}{|c|c|c|}
            ...
            \hline
            Assign ministerial inquiry & 2 days & 30 min \\
            Investigate ministerial inquiry & 8 days & 12 hours \\
            Prepare ministerial response & 4 days & 4 hours \\
            Review ministerial response & 4 days & 2 hours \\
            \hline
        \end{tabular}
    }
\end{table}
Using the information provided, compute the CTE (Cycle Time Efficiency) of the process.
\end{exercise}
\paragraph{Solution:}
To compute the CTE, we first need to compute the Cycle Time (CT) and Processing Time (PT) of the process.
\[CT = 2 + 8 + \frac{4 + 4}{0.8} + 4 = 20 \text{ days}\]
Remember that, if we have a structured loop with probability \(p\), the formula to compute the cycle time of that is:
\[
    \text{Cycle Time}_{\text{loop}} = \frac{\text{CT}_{\text{body}}}{p_{\text{exit}}}
\]
Similarly, we can compute the Processing Time (PT) using the same approach:
\[PT = 0.5 + 12 + \frac{4 + 2}{0.8} = 20 \text{hours}\]
Finally, we can compute the Cycle Time Efficiency (CTE) as:
\[CTE = \frac{PT}{CT} = \frac{20}{20 \times 8} = \frac{20}{160} = 12.5\%\]

\begin{exercise}{2}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/exercises/proc_analysis/q_a-2.png}
    \caption{Example Process for Quantitative Analysis Exercises}
\end{figure}
\begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/exercises/proc_analysis/q_a-3.png}
    \caption{Example Process for Quantitative Analysis Exercise 2}
\end{figure}
and the following table:
\begin{table}[H]
    \centering
    \resizebox{0.6\textwidth}{!}{%
        \begin{tabular}{|c|c|c|}
            ...
            \textbf{Activity} & \textbf{Cycle Time (CT)} & \textbf{Processing Time (PT)} \\
            \hline
            Decide shipment method      & 1 day    & 30 min    \\
            Check extra insurance       & 6 days   & 14 hours  \\
            Add stamps                  & 3.5 days & 6 hours   \\
            Check postage fee           & 1 day    & 3 hours   \\
            Request quotes from carriers& 6 days   & 30 min    \\
            Assign a carrier            & 5 days   & 8.5 hours \\
            Package goods               & 2 days   & 10 hours  \\
            Add paperwork               & 3 days   & 4 hours   \\
            \hline
        \end{tabular}
    }
\end{table}
Comput the Cycle Time Efficiency (CTE) of the process.
\end{exercise}
\paragraph{Solution:}
The Cycle Time (CT) can be computed as:
\[CT = \max\left(2, \left[1 + 0.7 \times \left(6 + \frac{3.5 + 1}{0.9} \right) + 0.3 \times \left(6 + 5 \right) \right] \right) + 3\]  
\[ = 1 + 0.7 \times (11) + 0.3 \times 11 + 3 = 1 + 7.7 + 3.3 + 3 = 15 \text{ days}\]
Similarly, the Processing Time (PT) can be computed as:
\[CT = \max\left(10, \left[0.5 + 0.7 \times \left(14 + \frac{6 + 13}{0.9} \right) + 0.3 \times \left(0.5 + 8.5 \right) \right] \right) + 4\]
\[ = 24 \text{ hours}\]
Finally, we can compute the Cycle Time Efficiency (CTE) as:
\[CTE = \frac{PT}{CT} = \frac{24}{15 \times 8} = \frac{24}{120} = 20\%\]  
